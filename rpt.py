...
def histograma(dias=None,desde=None,hasta=None,archivo='reporte.pdt'):
    ...
    for x in range(0,dias):
        ...
        horas.append(hh((hasta - datetime.timedelta(days=x))))
...
def main():
    ...
    p = a.parse_args()
    if p.histograma:
        histograma(desde=p.desde,hasta=p.hasta,dias=p.dias,archivo=p.archivo)
